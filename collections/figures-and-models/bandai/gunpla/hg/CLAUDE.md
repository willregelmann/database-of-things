...
are a rough guide): HGBD/Build Divers (30), HGFC/Future Century-G Gundam
...
sub-line has these — most (HGGS, HG00, HGI-BO, HGAGE, HGAC) have none,
and where present they usually surface nothing new (HGGTO's 2), though
HGBF's one turned up 8 genuine redlinks (deferred, not fabricated).
...
pattern, and always confirm the page's OWN template/infobox rather than
trusting a bare keyword hit.** Three known so far: `{{Canceled}}`/
...
or — as HGAC's `XXXG-01S Shenlong Gundam` showed — a kit's own Notes &
Trivia section describing its *history* as a long-delayed release before
it eventually shipped (confirmed real via its own `Lineup no.`/`Release
Date`). Always check the page's own infobox data, never just a keyword
